...
int get_Nombre(char * mensaje, char* cadena, int tamanio){
	...
	if(cadena != NULL && tamanio > 0)
	{
		...
		do{
			...
			fflush(stdin);
			scanf("%[^\n]", auxiliar);
			respuesta = esValidoDescripcion(auxiliar, tamanio);
		}while(respuesta == 0);
...
int esValidoDescripcion(char* auxiliar, int tamanio){
	...
	if(auxiliar != NULL)
	{
		...
		for(i = 0; i < largo; i++)
		{
			if((auxiliar[i] < 'A' || auxiliar[i] > 'Z') && (auxiliar[i] < 'a' || auxiliar[i] > 'z') && (auxiliar[i] < 0 || auxiliar[i] > 9) && auxiliar[i] != ' ')
			{
					retorno = 0;
					break;
			}
