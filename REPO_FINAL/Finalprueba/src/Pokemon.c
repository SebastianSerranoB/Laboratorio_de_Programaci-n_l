...
int Pokemon_mapearValorDeAtaque(void* elemento)
{
	int retorno;
	int auxValorAtaque;
	char auxTipo[100];
	...
	{
		pokemonAux = (ePokemon*)elemento;
		Pokemon_getTipo(pokemonAux, auxTipo);

		if(stricmp(auxTipo, "Fire") == 0 || stricmp(auxTipo, "Ground") == 0 || stricmp(auxTipo, "Grass") == 0)
		{
			Pokemon_getValorAtaque(pokemonAux, &auxValorAtaque);
			auxValorAtaque = auxValorAtaque *1.10;
			Pokemon_setValorAtaque(pokemonAux, auxValorAtaque);
			retorno = 0;
		}
...
//ELIMINO EL POKEMON Y LO RETORNO CON POP
int eliminoYMuestroPokemon(LinkedList* pListaPokemon)
{
	...
	if(ll_len(pListaPokemon) > 0)
	{
		numeroIngresado = getInt(1, 151, "\nIngrese el NUMERO de un pokemon a eliminar: ");
		for(i=0; i<ll_len(pListaPokemon); i++)
		{
			auxPokemon = (ePokemon*)ll_get(pListaPokemon, i);
			if(auxPokemon != NULL)
			{
				Pokemon_getNumero(auxPokemon, &numeroAuxiliar);
				if(numeroIngresado == numeroAuxiliar)
				{
					printOneElement(auxPokemon);
					ll_remove(pListaPokemon, i);
					retorno = 0;
					//(ePokemon*)ll_pop(pListaPokemon, i);  otra forma de hacerlo, resolvi mostrarlo antes y eliminarlo con remove
				}
			}
		}

	}else{
		printf("\nDebe haber elementos en lista para poder eliminarlos.");
	}
