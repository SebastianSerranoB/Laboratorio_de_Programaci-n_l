  ...
  //VALIDO QUE no sea null para cargar los campos,
    if(this != NULL)
    {
	 //inicializamos sus campos //valores default para construir
		//cuando la creemos, por supuesto, el tamaño va a ser 0
		this->size = 0;
...
int ll_len(LinkedList* this)
{
    ...
    if(this != NULL)
    {
    	//simplemente retorna el tamaño de la estructura
    	returnAux = this->size;
    }
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && index < ll_len(this))
    {
    	if(index == 0)
    	{
    		...
    		//free del elemento tmbn, se encarga el GARBAGE COLLECTOR, de liberar la memoria reservada para el elemento del nodo
    	...
    	else
    	{
			...
			if(pAuxNode != NULL && pNode != NULL)
			{
				...
				free(pNode);
				//el elemento tmbn , se encargaria el gabrage collector
				//free(pNode->pElement);
...
int ll_clear(LinkedList* this)
{
    ...
    //hay que hacerle un free al elemento tambien, corresponde hacerlo en ll_remove
    if(this != NULL)
    {
...
//recibe la ll original, la clona, pasa por func criterio un elemento traido del  clon y luego retorna la lista mapeada(clon)
LinkedList* ll_map(LinkedList* this, int (*pFunc)(void*))
{
	...
	if(this != NULL && pFunc != NULL)
	{
		mapedList = ll_clone(this);
		if(mapedList != NULL)
		{
			for(i=0;i<ll_len(this);i++)
			{
				pElement = (void*)ll_get(mapedList,i);

				//0 == OK
				if(pFunc(pElement) == 0)
				{
					//estoy corroborando que el elemento se guarde en la lista y no se pierda en la pFunc
					//ll set guarda el elemento en la posicion i, en la lista maped
					ll_set(mapedList, i, pElement);
				}
			}
		}
...
//retorna cantidad numerica de elementos que pasan el criterio de funcion recibida como parametro
