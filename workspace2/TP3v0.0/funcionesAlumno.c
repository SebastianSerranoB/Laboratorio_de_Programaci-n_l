...
int listPassengers(LinkedList* pArrayPassengers)
{
	...
	if(pArrayPassengers != NULL)
	{
		...
		for(i=0;i<limite;i++)
		{
			...
			//printf("\nID: %d\tNOMBRE: %s\tAPELLIDO: %s\tPRECIO: %f\tTIPO: %d\tCODIGO: %s\tESTADO: %d",passengerAux->id, passengerAux->nombre, passengerAux->apellido, passengerAux->precio, passengerAux->tipoPasajero, passengerAux->codigoVuelo, passengerAux->estadoVuelo);
...
int printOnePassenger(Passenger* this)
{
	...
	if(this != NULL)
	{
		printf("LOS GETTERS PASARON");
		if(Passenger_getId(this, &idAux) == 0 && Passenger_getNombre(this, nombreAux) == 0
				&& Passenger_getApellido(this, apellidoAux) == 0 && Passenger_getPrecio(this, &precioAux) == 0
				&& Passenger_getTipoPasajero(this, &tipoPasajeroAux) == 0
				&& Passenger_getCodigoVuelo(this, codigoVueloAux) == 0 && Passenger_getEstadoVuelo(this, &estadoVueloAux) == 0 )
		{

			printf("\n %5d\t  %15s\t  %15s\t  %10.2f\t  %15d\t  %10s\t  %15d",idAux, nombreAux, apellidoAux, precioAux, tipoPasajeroAux, codigoVueloAux, estadoVueloAux);
			rtn = 0;
		}
	}
