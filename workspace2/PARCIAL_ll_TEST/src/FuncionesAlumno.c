...
int ordenarPorNombre(void* pElementA, void* pElementB)
{
	...
	{
		empleadoUno = (Empleado*)pElementA;
		empleadoDos = (Empleado*)pElementB;

		Empleado_getNombre(empleadoUno, auxNombreA);
		Empleado_getNombre(empleadoDos, auxNombreB);
...
int ordenarPorApellido(void* pElementA, void* pElementB)
{
	...
	if(pElementA != NULL && pElementB != NULL)
	{
		...
		Empleado_getApellido(empleadoUno, auxApellidoA);
		Empleado_getApellido(empleadoDos, auxApellidoB);
...
int ordenarPorPrecio(void* pElementA, void* pElementB)
{
	...
	if(pElementA  != NULL && pElementB != NULL)
	{
		...
		Empleado_getPrecio(empleadoUno, &auxPrecioA);
		Empleado_getPrecio(empleadoDos, &auxPrecioB);
...
int ordenarPorId(void* pElementA, void* pElementB)
{
	...
	if(pElementA != NULL && pElementB != NULL)
	{
		...
		Empleado_getId(empleadoUno, &auxIdA);
		Empleado_getId(empleadoDos, &auxIdB);
...
int Empleado_descuento(void* pElement)
{
	...
	if(pElement != NULL)
	{
		pAuxElement = (Empleado*)pElement;
		Empleado_getPrecio(pAuxElement, &auxPrecio);
		if(auxPrecio < 10000)
		{
			rtn = 0;
		}
...
int salirSinGuardar(void)
{
	...
	printf("\nGuarde los datos en formato binario y texto");
	printf("\nLos datos no guardados se perderan..");
	printf("\nIngrese 1 para volver al menu principal y guardar(OPCION 7 Y 8).");
	respuesta = getInt(0, 1, "\n¿Salir sin guardar? [S= 0/ N = 1]: ");
